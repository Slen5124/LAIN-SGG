...
class PromptLearner(nn.Module):
    def __init__(self, args, classnames, clip_model):
        ...
        # Runtime S-P-O sentences require token embeddings whose weights stay
        # frozen. A non-persistent buffer follows the module device without
        # changing optimizer or checkpoint contracts.
        self.register_buffer(
    ...
    def forward_dynamic(
        ...
        """Assemble runtime S-P-O text with learned predicate contexts."""
        # [VG online literal text]
        # The literal suffix changes for every S-O pair, while class_indices
        # selects the corresponding predicate context. With OvR CSC=False,
        # all 50 predicates share the Base-trained context.
...
class CustomCLIP(nn.Module):
    ...
    def encode_dynamic_text(
        ...
        """Encode trainable literal S-P-O prompts in bounded chunks."""
        # [VG online literal text]
        # Chunking controls activation memory. Gradient checkpointing reruns
        # the frozen text transformer during backward while retaining gradient
        # flow into the learned prompt context.
        if chunk_size <= 0:
            raise ValueError(
        ...
        for start in range(0, len(classnames), chunk_size):
            stop = min(start + chunk_size, len(classnames))
